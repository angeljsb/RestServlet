...
/**
 * Clase que permite leer los parametros en la request a un servlet. Permite
 * leer request body de tipo JSON y obtener los parametros en los distintos
 * tipos nativos, además de obtener cookies y headers de la request
 *
 * @since v1.0.0
 * @author Angel
 */
public class RestRequestReader {
    ...
    public RestRequestReader(HttpServletRequest request) {
        ...
        if (content != null && content.contains(MediaType.MULTIPART_FORM_DATA)) {
            try {
                Collection<Part> parts = this.httpServletRequest.getParts();
                Map<String, List<String>> multipartMap = new HashMap();
                for (Part part : parts) {
                    if (part.getHeader("Content-Type") != null){
                        continue;
                    }
                    String name = part.getName();
                    String value = readInputStream(part.getInputStream(), request.getCharacterEncoding());
                    List<String> vals;
                    if (multipartMap.containsKey(name)){
                        vals = multipartMap.get(name);
                    } else {
                        vals = new ArrayList();
                        multipartMap.put(name, vals);
                    }
                    vals.add(value);
                }
                multipartMap.forEach((key, value) -> {
                    if (value.size() == 1) {
                        this.parameterMap.put(key, value.get(0));
                    } else {
                        this.parameterMap.put(key, new JSONArray(value).toString());
                    }
                });
    ...
    /**
     * Funciona para añadir parametros extra al lector que no han sido leídos
     * inicialmente. Esto funciona, por ejemplo, para los parametros en el
     * pathInfo, que no se pueden leer sin la ayuda del Servlet, así que el
     * servlet se puede encargar de leerlos y agregarlos al objeto
     * RestRequestReader
     *
    ...
    /**
     * Obtiene un mapa con todos los parametros y sus valores como String. Los
     * parametros que sean tipos nativos estarán como su representación en
     * String y deberán parsearse. Los arreglos estarán en formato JSONArray
     * {@code [...values]} y pueden ser obtenidos por medio del constructor
     * {@link org.json.JSONArray#JSONArray(java.lang.String) JSONArray(String)}
     *
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code byte}. Si falla en
     * obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
     * en obtener el parametro
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code int}. Si falla en
     * obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code long}. Si falla en
     * obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code float}. Si falla
     * en obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
     * en obtener el parametro
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code double}. Si falla
     * en obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code boolean}. Si falla
     * en obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
     ...
     * @param key El nombre del parametro
     * @return El valor del parametro
     * @throws RestException Si el parametro no existe. Si se quiere prevenir
     * esto se puede usar 
    ...
    /**
     * Obtiene un parametro de la request convertido a {@code String}. Si falla
     * en obtener el parametro devuelve el valor por defecto
     *
     * @param key El nombre del parametro
     * @param def El valor por defecto, que será devuelto en caso de que falle
    ...
    /**
     * Si se detecta que un parametro tiene más de un valor o el parametro llega
     * en un json en forma de json array, este puede ser obtenido como un objeto
     * {@link org.json.JSONArray} por medio de esta función.
     *
     * @param key El nombre del parametro
     * @return Los valores del parametro en un JSONArray
     * @throws RestException Si el parametro no existe o no puede ser convertido
     * en arreglo. Si se quiere prevenir esto se pueden usar los metodos
     * {@link #getArray(java.lang.String, java.lang.String) getArray(String, String)}
     * o
     ...
     *
     * @param key El nombre del parametro
     * @param def El String que representa el JSONArray por defecto. Ej:
     * {@code "[]"}
     * @return Los valores del parametro en un JSONArray o el string por defecto
     * convertido a JSONArray
     */
    ...
    /**
     * Obtiene el valor de un parametro como un json. Aunque se recomienda no
     * pasar parametros de este tipo, es posible agregar en el body de una
     * request un parametro que sea un json. En ese caso, se puede obtener con
     * este metodo
     *
     * @param key El nombre del parametro
     * @return El parametro como {@link org.json.JSONObject}
     * @throws RestException Si el parametro no existe o no puede ser convertido
     * en json. Si se quiere prevenir esto se pueden usar los metodos
     * {@link #getJSON(java.lang.String, java.lang.String) getJSON(String, String)}
     * o
    ...
    /**
     * Obtiene el valor de un parametro en un objeto {@link org.json.JSONObject}
     * o un objeto por defecto si no es posible
     *
     * @param key El nombre del parametro
     * @param def El objeto por defecto
     * @return El valor del parametro en un JSONObject o el objeto por defecto
     * convertido a JSONObject
     */
    ...
    /**
     * Obtiene el valor de un parametro en un objeto {@link org.json.JSONObject}
     * o un json por defecto si no es posible
     *
     * @param key El nombre del parametro
     * @param def El String que representa el JSONObject por defecto. Ej:
     * {@code "{}"}
     * @return El valor del parametro en un JSONObject o el string por defecto
     * convertido a JSONObject
     */
     ...
     * request does not have a header of the specified name, this method returns
     ...
     * @return La cookie cuyo nombre coincida con el buscado o null si o se
     * encuentra
     */
